...
class HomeWorkTests(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.cat1 = Category.objects.create(slug='random', label='Some random news')
        cls.cat2 = Category.objects.create(slug='programming', label='Programming news')
        cls.cat3 = Category.objects.create(slug='animal', label='Animals news')
    ...
    def test_categories(self):
        ...
        self.assertEqual(3, len(response.context['categories']))

    def test_unknown_post(self):
        post_detail_url = reverse('post-detail', kwargs=dict(post_id=0))
...
class SearchBoxManager(TestCase):
    ...
    def setUpTestData(cls):
        category = Category.objects.create(slug='random', label='Random News')
        cls.category = category
        cls.juan = User.objects.create(first_name='juanito', is_active=True, username='oscar')
        author = CoolUser.objects.create(user=cls.juan)
        cls.author = author
        for title, body in zip(TITLES, BODIES):
            _ = Post.objects.create(author=author, category=category, title=title, body=body)
...
class MediaStackManager(TestCase):
    ...
    def test_get_mediastack_sport_posts(self):
        ...
        posts_created = gather_and_create_news(categories, languages, limit)
        self.assertLess(len(posts_created), limit)
